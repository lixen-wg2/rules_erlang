...
-spec compile_apps(
    pid(), [module()], #{atom() := target()}, string(), [string()], module_index(), inputs()
) -> ok.
compile_apps(
    NotifyPid, OrderedApplications, Targets, DestDir, CodePaths, ModuleIndex, MappedInputs
) ->
    R = lists:foldl(
        fun
            (_, {_, {error, _, _}} = E) ->
            ...
            (AppName, {ModulesSoFar, {ok, Warnings}}) ->
                ...
                case compile(AppName, Targets, DestDir, CodePaths, ModuleIndex, MappedInputs) of
                    ...
                    {error, M, Errors, W} ->
                        {ModulesSoFar ++ M, {error, {AppName, Errors}, Warnings ++ [{AppName, W}]}}
                end
        end,
        {[], {ok, []}},
        OrderedApplications
...
-spec resolve_include_lib(file:name(), string(), [string()], atom(), #{atom() := target()}, [
    string()
]) ->
    ok | {ok, file:name()} | {warning, term()}.
                    ...
                    case Targets of
                        ...
                        _ ->
                            ...
                            of
                                ...
                                _ ->
                                    {warning,
                                        {include_lib_application_missing, IncludeLib, OtherAppName}}
                            end
                    end;
                R ->
...
-spec src_graph(atom(), [file:name()], [compile:option()], module_index(), inputs()) ->
    digraph:graph().
